...
Using ggplot (data = penguins) R will give us a blank chart again. This is because we have not been specific with R and told it what we want to plot and how.

# Create first plot.
```{r}
ggplot(data = penguins,  # Specify data we want to use.
...
```{r}
ggplot(
  data = penguins,  # Specify data we want to use.
...
ggplot(
  ...
  geom_smooth(method = "lm") + # Add a line of best fit to entire data set, method = 'lm' tells R to fit a linear relationship for line of best fit.
  labs(
    title = "Body mass and flipper length", # Create title for plot..
...
ggplot(data = penguins) + # Specify data set.
  ...
  facet_wrap(~ species, nrow = 2) # This function makes a plot that represents multiple plots which we separated out by species. This now shows three plots for each species. The nrow function tells R how many rows we want on the overall plot.
```
...
```{r}
ggplot(data = penguins) +  # Specify data we want to use.
  geom_point(mapping = aes(x = body_mass_g, y = flipper_length_mm)) + # Specify mapping aesthetics
  facet_grid( sex ~ species) # Create facet grid and separate out by sex and species.
```
...
```{r}
ggplot(data = penguins) +  # Specify data we want to use.
  geom_point(mapping = aes(x = body_mass_g, y = flipper_length_mm)) + # Create mapping aesthetics
...
# scales adjusts the x and y scales. These can be fixed which is the default, or free. When using the free option we can also specify if this is specifically for the x or y axis. In the above example we made the x axis free.
...
```{r}
ggplot(penguins, aes(x = flipper_length_mm, y = body_mass_g)) + # Specify mapping aesthetics.
  geom_point(aes(colour = species, shape = species)) + # Set aesthetics to the points.
  facet_wrap(~island) # Create facet wrap. Separate by island.
```
...
```{r}
ggplot(data = penguins) +  # Specify data we want to use.
  geom_smooth(mapping = aes(x = body_mass_g, y = flipper_length_mm)) # Set arguments for x and y axes. 
```
...
## Using comments (#)
wecna comment out sections of our coded and leave a note to ourselves to know what we have tried. By doing this we can have a history of what we have attempted and why we decided it was not useful. The below example shows that we attempted a plot using points but this was not a good representation so instead we tried a smooth plot. 
```{r}
ggplot(data = penguins) +  # Specify data we want to use.
...
## Question:
How does R work out where to plot the line? Can use the chunk output to inform us. Can also use the help menu.

Using the chunk output for the above example we can see that the geom_smooth is using the loess method. When searching this in the help bar this function is explained as the way R determines how to fit the data. There are different kinds of smoothing functions and the one selected depends on the size of the data. In this case using loess is used for datasets with less than 1000 observations.


# Changing linetype
The linetype function is used to change the settings of the lines on our plots. Below we specify the linetype as species which tells R to change the line aesthetics for each species type. We can also use linetype to specify the type of line we want rather than changing it for each species. For example, we could use solid for solid lines or specify dash if we wanted dashed lines.
```{r}
ggplot(data = penguins) + # Specify data we want to use.
  geom_smooth(mapping = aes(x = body_mass_g, y = flipper_length_mm, linetype = species)) # Specify linetype. 
```


# Grouping
You can use the group function to set group aesthetics for a variable. for example when we tell R that we want to group the geom_smooth by species, this will create an individual smooth line for each species type. This differs to linetype as well because it does not add a legend or any other distinguishing features.
```{r}
ggplot(data = penguins) + # Specify data we want to use.
  geom_smooth(mapping = aes(x = body_mass_g, y = flipper_length_mm, group = species)) # Specify grouping to species.
```
 
# Change line colour based on species value
Different to the group function, we can adjust colours of the geom to a specific category and this will also create multiple lines only this time the colour will vary. This will normally generate a legend which is also different to the group function but in the below example we told R not to include one.
```{r}
ggplot(data = penguins) +# Specify data we want to use.
  geom_smooth(
    mapping = aes(x = body_mass_g, y = flipper_length_mm, color = species), # Using color =  we tell R to change line colour for each species.
...
```{r}
ggplot(data = penguins) + # Specify data we want to use.
  geom_point(mapping = aes(x = body_mass_g, y = flipper_length_mm)) + # Set mapping aesthetics for point geom.
  geom_smooth(mapping = aes(x = body_mass_g, y = flipper_length_mm)) # Set mapping aesthetics for smooth geom.
```

A better way to do this is demonstrated below:
```{r}
ggplot(data = penguins, mapping = aes(x = body_mass_g, y = flipper_length_mm)) + # Specify data we want to use and set mapping aesthetics.
  geom_point() + # Specify geom we want.
  geom_smooth() # Specify second geom we want.
```
These give the exact same figure the coding is just slightly different. 

# Can still manipulate each geom/layer separately:
Using the above example, we can also manipulate the geoms to give more information on our plots. The below example demonstrates how we can alter the points on our plot to give more information on the species differences.
```{r}
ggplot(data = penguins, mapping = aes(x = body_mass_g, y = flipper_length_mm)) + # Specify data we want to use and set mapping aesthetics.
  geom_point(mapping = aes(color = species)) + # Change geom point aesthetics to set colour dependent on species type.
  geom_smooth() # Add geom smooth to plot to show trends in data.
```
...
ggplot(data = penguins, mapping = aes(x = body_mass_g, y = flipper_length_mm)) + # Specify data we want to use and set mapping aesthetics.
  geom_point(mapping = aes(color = species)) + # Change geom point aesthetics to set colour dependent on species type.
  geom_smooth(data = filter(penguins, species == "Gentoo"), se = FALSE) # The filter in this tells R we only want to use a geom smooth for the Gentoo species. USing se= False tells R we want to remove the confidence interval around the line.
```
...
Run a density plot rather than the line chart (other times we can just use geom_line but this is not useful for this data. We could use geom_smooth but this was done above so I chose density for something different). Run a boxplot, histogram and a bar chart.

```{r}
# Density plot:
ggplot(penguins, aes(x = body_mass_g, colour = species)) + # Specify data we want to use and set mapping aesthetics.
  geom_density(linewidth = 0.75) # Specify geom type.

#we can map species both to color and fill aesthetics and to make the fill transparent use the alpha aesthetic.
ggplot(penguins, aes(x = body_mass_g, colour = species, fill = species)) + # Specify data we want to use and set mapping aesthetics.
  geom_density(alpha = 0.5) # Alter transparency of fill.

# Boxplot:
ggplot(penguins, aes(x = species, y = body_mass_g)) + # Specify data we want to use and set mapping aesthetics.
  geom_boxplot() # Specify geom type.
...
Sometimes we want to plot multiple bits of information on one plot. We can do this by using different aesthetics.

In the below chart we represent the island by shape and represent the species by colour.

```{r}
ggplot(penguins, aes(x = flipper_length_mm, y = body_mass_g)) + # Specify data we want to use and set mapping aesthetics.
  geom_point(aes(color = species, shape = island)) # Set colour aesthetics to species and shape aesthetics to island.
```
...
# Bar plot
```{r}
ggplot(data = diamonds) + # Specify data.
  geom_bar(mapping = aes(x = cut)) # Set mapping aesthetics using geom.
```

```{r}
ggplot(data = diamonds) + # Specify data.
  stat_count(mapping = aes(x = cut)) # Set mapping aesthetics using stat instead of geom.
```
...
Make some new data
```{r}
demo <- tribble( # Make new data set using tribble.
  ~cut,         ~freq,
  "Fair",       1610,
  "Good",       4906,
  "Very Good",  12082,
...
```{r}
ggplot(data = demo) + # Specify data.
  geom_bar(mapping = aes(x = cut, y = freq), stat = "identity") # Set mapping aesthetics.
```
...
```{r}
ggplot(data = diamonds) + # Specify data we want to use.
  geom_bar(mapping = aes(x = cut, y = stat(prop), group = 1)) # Use stat prop to get a proportion of the total diamond dataset rather than a count which is what stat = identity gives.
```
...
```{r}
ggplot(data = diamonds) + # Specify data we want to use.
  geom_bar(mapping = aes(x = cut, y = stage(after_stat = prop), group = 1)) # Get the proportion of diamond data set using after_stat = prop.
...
```{r}
ggplot(data = diamonds) + # Specify data.
  geom_bar(mapping = aes(x = cut, colour = cut)) # Set mapping aesthetics, set colour to cut. This will do the outline.

ggplot(data = diamonds) + 
  geom_bar(mapping = aes(x = cut, fill = cut)) # Set mapping aesthetics, set fill to cut. This will ensure the bars are a solid block colour depending on the cut type.
```
The above charts are using the viridis colour package which is why the colour scheme is different. this is demonstrated below. The first graphuses the default colour package and the second chart uses the viridis colour package for colour blindness.

Check out the difference:
```{r}
ggplot(data = mpg, mapping = aes(x = displ, y = hwy)) + # Specify data and set mapping aesthetics.
  geom_point(mapping = aes(color = class)) # Select geom type and make colour = class.

ggplot(data = diamonds, mapping = aes(x = color, y = depth)) + # Specify data and set mapping aesthetics.
  geom_point(mapping = aes(color = cut))  # Select geom type and make colour = cut.
```
...
```{r}
ggplot(data = diamonds) + # Specify data.
  geom_bar(mapping = aes(x = cut, fill = clarity)) # Set mapping aesthetics, make fill = clarity.
```


# To alter transparency (alpha)
```{r}
ggplot(data = diamonds, mapping = aes(x = cut, fill = clarity)) + # Specify data and set mapping aesthetics.
  geom_bar(alpha = 1/5, position = "identity") # Use alpha function too adjust the transparency of the fill which is set to clarity.
```


# To color the bar outlines with no fill color
```{r}
ggplot(data = diamonds, mapping = aes(x = cut, colour = clarity)) + # Specify data and set apping aesthetics so colour = clarity.
  geom_bar(fill = NA, position = "identity") # fill = NA tells R to not fill geom so we only have coloured outlines to what we have specified in the previous line of code.
```
...
```{r}
ggplot(data = diamonds) + # Specify data.
  geom_bar(mapping = aes(x = cut, fill = clarity), position = "fill") # Set mapping aesthetics.
```
...
```{r}
ggplot(data = diamonds) + # Specify data
  geom_bar(mapping = aes(x = cut, fill = clarity), position = "dodge") # Set mapping aesthetics.
```
...
```{r}
ggplot(mpg, aes(displ, hwy)) + # Specify data and select variables of interest.
  geom_point(aes(colour = class)) + # Set mapping aesthetics
  geom_smooth(se =  FALSE) + # Remove the confidence intervals from plot.
  labs(title = "Fuel efficiency generally decreases with engine size") # Create Plot title.
```
...
```{r}
ggplot(mpg, aes(displ, hwy)) + # Specify data.
  geom_point(aes(colour = class)) + # Set mapping aesthetics.
  geom_smooth(se =  FALSE) + # Remove confidence intervals.
  labs(title = "Fuel efficiency generally decreases with engine size", # Create title.
       subtitle = "Two seaters (sports cars) are an exception because of their light weight", # Create subtitle
       caption = "Data from fueleconomy.gov") # Create caption.
```
...
ggplot(mpg, aes(displ, hwy)) + # Specify data.
  ...
  geom_smooth(se = FALSE) + # Remove confidence intervals.
  labs(
    x = "Engine displacement (L)", # Create label for x axis.
    y = "Highway fuel economy (mpg)", # Create label fro y axis.
    colour = "Car type" # Create legend title.
  ) 
...
```{r}
best_in_class <- mpg %>% # Use mpg data to create new data frame.
  group_by(class) %>% # group by class.
  filter(row_number(desc(hwy))==1) # desc function means descending order. 

ggplot(mpg, aes(displ, hwy)) + # Specify data
  geom_point(aes(colour = class)) + # Set mapping aesthetics.
  geom_text(aes(label = model), data = best_in_class) # USe geom_text to create annotations on the chart.
```
...
ggplot(mpg, aes(displ, hwy)) + # Specify data.
  geom_point(aes(colour = class)) + # Set mapping aesthetics.
  scale_x_continuous(limits = c(1, 8)) + # Change x axis limits. 
  scale_y_continuous(limits = c(10, 45)) + # Change y axis limits. 
  scale_colour_discrete() # Select colour package.
...
ggplot(mpg, aes(displ, hwy)) + # Specify data
  geom_point(aes(colour = class)) + # Set mapping aesthetics.
  scale_y_continuous(breaks = seq(15, 40, by = 5)) # Adjust y axis limits.
...
ggplot(mpg, aes(displ, hwy)) + # Specify data.
  geom_point(aes(colour = class)) + # Set mapping aesthetics.
  scale_x_continuous(labels = NULL) + # Remove axis tick labels from x axis.
  scale_y_continuous(labels = NULL) # Remove axis tick labels from y axis.
```
...
```{r}
base <- ggplot(mpg, aes(displ, hwy)) + # Specify data.
        geom_point(aes(colour = class)) # Set mapping aesthetics.
...
```{r}
ggplot(diamonds, aes(carat, price)) + # Specify data.
...
ggplot(mpg, aes(displ, hwy)) + # Specify data.
  geom_point(aes(colour = drv)) +  # Set mapping aesthetic.
  scale_colour_brewer(palette = "Set2") # Change colour package. 
...
ggplot(mpg, aes(displ, hwy)) +# Specify data.
  ...
  scale_colour_brewer(palette = "Set1") # Select colour package.
```
If you are using predefined colours you are wanting to set yourself use scale_colour_manual().
```{r}
presidential %>% # Specify data.
  mutate(id = 33 + row_number()) %>%  # Wrangle data.
  ggplot(aes(start, id, colour = party)) + # Set mapping aesthetics
  geom_point() + # Set mapping aesthetics
  geom_segment(aes(xend = end, yend = id)) + # Set mapping aesthetics.
  scale_colour_manual(values = c(Republican = "red", Democratic = "blue")) # Manually set colours for specified values.
```
...
ggplot(df, aes(x, y)) + # Sepecify data.
  geom_hex() + # Set mapping aesthetic.
  coord_fixed() # Set a fixed aspect ratio for x and y axes.
...
```{r}
ggplot(mpg, aes(displ, hwy)) + # Specify data.
  geom_point(aes(color = class)) + # Set mapping aesthetics.
  geom_smooth(se = FALSE) + # Remove confidence interval.
  theme_bw() # Selct theme.

ggplot(mpg, aes(displ, hwy)) +# Specify data.
  geom_point(aes(color = class)) + # Set mapping aesthetics.
  geom_smooth(se = FALSE) + # Remove confidence interval.
  theme_light() # Selct theme.

ggplot(mpg, aes(displ, hwy)) + # Specify data.
  geom_point(aes(color = class)) + # Set mapping aesthetics.
  geom_smooth(se = FALSE) + # Remove confidence interval.
  theme_classic() # Selct theme.

ggplot(mpg, aes(displ, hwy)) + # Specify data.
  geom_point(aes(color = class)) + # Set mapping aesthetics.
  geom_smooth(se = FALSE) + # Remove confidence interval.
  theme_dark() # Selct theme.
...
For example:
```{r}
 theme (panel.border = element_blank(), #Adjust border.
        panel.grid.minor.x = element_blank(), #  Adjust minor grid lines for x axis.
        panel.grid.minor.y = element_blank(), # Adjust minor grid lines for y axis.
        legend.position="bottom", # adjust legend position.
        legend.title=element_blank(), # Adjust legend title.
        legend.text=element_text(size=8), # Adjust text size.
        panel.grid.major = element_blank(), # Adjust major grid lines.
        legend.key = element_blank(), # Adjust legend key.
        legend.background = element_blank(), # Adjust background.
        axis.text.y=element_text(colour="black"), # Select text colour for y axis.
        axis.text.x=element_text(colour="black"),# Select text colour for x axis.
        text=element_text(family="Arial")) # Select font.
```
...
```{r}
library(tidyverse) # Load required
```
...
table3
```
Table 1 shows four different columns these are country, year, cases and population. The observations in this table give numeric data for the population column which provides information on the population size of a country at a given year. The cases columnn is also numeric observations showing the total cases that occurred in a country for a given year.


Table 2 also has four columns, country, year, type and count. In this case the column type has two main observations which are cases and population. The count column has numeric data that corresponds to the type column and varies between counts of cases and counts of population.

Table 3 has only three columns, country year and rate. Rate is a combined number of cases and population which is separated using a / to determine where one number ends and where the other begins.

Of all these tables only Table 1 is tidy. This is because it can easily be worked with in R where the following tables are more complicated and will need to be better organised. Particularly table 3. The rate column is not provided in a way that allos us to work with it for any kind of analysis or visualisation.

# Question 2:
Sketch out the processes you would use  to calculate the rate for table 2 and table 3. you will need to perform the four operations:
  a) Extract the number of TB cases per country per year.
  b) Extract the matching population per country per year.
  c) Divide cases by population, and multiply by 10,000.
  d) Store back in the appropriate place.
  
For table 2 we will have to separate the column type into a seperate column for cases and one for population with the correct values placed into each of those columns from the count column. From this we can then follow a similar method outlined in the chunk below as was done for table 1.

For table 3 we need to create a column for the different values. We could easily separate these using the / as a way to determine where to split the data into separate columns titled cases and population. Then follow a similar method as in the chunk below.

  
```{r}
# Using table1, compute rate per 10,000
table1 %>% 
  mutate(rate = cases / population * 10000) # mutate allows us to create new variables.

# Using table1, compute cases per year
table1 %>% 
  count(year, wt = cases) # Creates a total sum of values using count function for each year.

# Visualise changes over time
library(ggplot2)
ggplot(table1, aes(year, cases)) + # Specify data.
  geom_line(aes(group = country), colour = "grey50") + # Set mapping aesthetics.
  geom_point(aes(colour = country)) # set mapping aesthetics.
...
billboard |>
  pivot_longer(
    cols =  starts_with("wk"), # This line of code is telling which columns need to be reorganised.
    names_to = "Week", # This tells R that the reshaped columns will be organised under the new column called Week.
    values_to = "Rank" # This function tells R that we want to create a new column called Rank for the data that was stored in the cells of the original columns.
  )
```
...
billboard |>
  pivot_longer(
    cols =  starts_with("wk"), # Select column to reorganise.
    names_to = "Week", # Create name for reshaped column.
    values_to = "Rank", # Create a new column for the values stored in original columns.
...
df |> # |> is a piping function.
  pivot_longer(
    cols = bp1:bp2, # This line of code is telling which columns need to be reorganised, using the : we tell R that all columns between and including bp1 and bp2 are being reshaped.
...
```{r}
cms_patient_experience |>
  distinct(measure_cd, measure_title) # Select a specific row/column using distinct function
```
...
cms_patient_experience |>
  pivot_wider( # Widen data set.
    id_cols = starts_with("org"), # Select columns that start with org
    names_from = measure_cd, 
    values_from = prf_rate
  )
```
...
df |> 
  ...
  pull() # pull() selects a column in a data frame and transforms it into a vector.
...
# Lengthening the data set there are three columns and four rows. This is because all the values are placed into one column stacking each value. 

stocks %>% 
  pivot_wider(names_from = year, values_from = return)
# Using wider we get three columns and 2 rows. The years are split into two separate columns. In this example the half column has only one row for value 1 and value 2 where the longer version has two rows for value 1 and two rows for value 2.
...
This code fails because it doesn't register 1999 and 2000 as columns so it doesn't believe they exist.
...
```
We could make this table longer by putting male and female into one column called sex and the numbers below as results in another column. For example the way this can be done is demonstrated below:
```{r}
preg %>%
  pivot_longer(
    cols = "male":"female", 
...
table3 %>%
  separate(rate, into = c("cases", "population")) # separate() will take the name of the column we want to split and the names of the columns we want it to split into.
...
```{r}
treatment |>
  fill(everything()) # Fill empty rows.
```
...
x * 10
x == 1
is.na(x) # This can tell both Na and NaN are NA values
is.nan(x) # In cases where we need to be specific we can use this function. This shows FALSE TRUE  because it is specifically identifying NaN values.
```
...
stocks |>
  complete(year = 2019:2021, qtr) # Generate missing values in special cases
```
...
```{r}
students <- read_csv("https://pos.it/r4ds-students-csv", na = c("N/A", "")) # Make N/A and empty cells equal NA values. 
students
```
...
students |> 
  rename(
    student_id = `Student ID`, # Remove backticks from column name and rename variable.
    full_name = `Full Name` # Remove backticks from column name and rename variable.
  )
```
...
read_csv("a,b\n1,2,3\n4,5,6") # The first line is used for data columns but the following 2 chunks of code have three variables not 2 so R will join the last two numbers of each chunk  together for the second column rather than having them as separate variables.
read_csv("a,b,c\n1,2\n1,2,3,4") # The 3 and 4 join together in the last row of the last column because there are more variables than there are columns.
read_csv("a,b\n\"1") # The "1" makes R unable to read the inline csv.
read_csv("a,b\n1,2\na,b") # The letters in the last chunk will put letters in the last row instead of numbers.
read_csv("a;b\n1;3") # Using a semicolon instead of a comma joins the a and b and the 1 and 3 together instead of separating them.
...
Relational data is a collection of multiple data tables in a given dataset that are related in some way. The term relational comes from the relations between the tables not just in the individual tables.
...
airlines # Shows the full carrier name from its abbreviated code.
...
weather %>%
  count(year, month, day, hour, origin) %>% # Count year, month, day, hour and origin.
  filter(n > 1)
...
# Mutating Joins
Mutating join is a tool to combine a pair of tables.

join functions add variables to the right side of the data table.
```{r}
flights2 <- flights %>% 
  select(year:day, hour, origin, dest, tailnum, carrier) # Create a subset of the data.

```


Use the mutating function left_join()
```{r}
flights2 %>%
  select(-origin, -dest) %>%  # Remove origin and dest variables.
  left_join(airlines, by = "carrier") # Use left join to join datasets by keeping ll observation in x.

```


The mutate() function can create the same result but is more involved which can become messy when matching multiple variables.
```{r}
flights2 %>%
  select(-origin, -dest) %>% # Remove origin and dest variables.
  mutate(name = airlines$name[match(carrier, airlines$carrier)]) # Tell R where we want to join the dataset.
```

Create a two small datasets to visualise the joins.
```{r}
x <- tribble( # Create a dataset.
  ~key, ~val_x,
     1, "x1",
     2, "x2",
     3, "x3"
)
y <- tribble( # create a dataset.
  ~key, ~val_y,
     1, "y1",
     2, "y2",
     4, "y3"
...
The inner join is the simplest join as it matches observations with equivalent keys.

```{r}
x %>% 
  inner_join(y, by = "key") # Join x and y by key variable using the inner_join function.
```
...
left_join(x, y, by = "key") # Keep all observations in x
```
...
```{r}
flights2 %>% 
  left_join(weather) # Join by weather.
```

```{r}
flights2 %>% 
  left_join(planes, by = "tailnum") # Joing flights2 and planes datasets using tailnum.
```

```{r}
flights2 %>% 
  left_join(airports, c("dest" = "faa")) # This will match dest variable in table flights2 to variable faa in dataset airports. 
```

```{r}
flights2 %>% 
  left_join(airports, c("origin" = "faa")) # This will match origin variable in table flights2 to variable faa in dataset airports. 
```
...
This is how we could write a workflow.
```{r}
# Step 3:
bop(
  scoop(
...
```
The main disadvantage of this method is that you have to read from inside-out, from right-to-left, and that the arguments end up spread far apart. This makes it harder for us to read and digest easily. 

We can make this more readable by using pipe functions
...
Things to Note when using pipes:

If your pipes are longer than  ~ten steps. Create intermediate objects with meaningful names. That will make debugging easier and it makes it easier to understand your code.

If you have multiple inputs or outputs. If there isn’t one primary object being transformed, but two or more objects being combined together, don’t use the pipe.

If you are starting to think about a directed graph with a complex dependency structure. Pipes are fundamentally linear and expressing complex relationships with them will typically yield confusing code.
...
```{r}
library(ggplot2)
ggplot(dat) + # Specify data.
  aes(x = longitude, y = latitude, colour = richness_raw) + # Set mapping aesthetics.
  geom_point() # Select geom.
```
Shows the location of every segment and colour the points by species richness. The axes are set up as latitude and longitude just like a map. However, it is not a map because it is lacking important map elements. In reality we have just got a scatter plot and a easy way to look at our data.

Visualise richness in a non-spatial way with latitude on the x-axis and richness on the y-axis.
```{r}
ggplot(dat, aes(x = latitude, y = richness_raw)) + # Specify data and set mapping aesthetics.
  stat_smooth() + # Select Stat type.
  geom_point() # Select geom.
```


# Getting going with maps
First we need to turn our point data into a spatially referenced data frame using the sf package (sf stands for ‘simple features’) which is an open standard for geospatial databases.
```{r}
sdat <- st_as_sf(dat, coords = c("longitude", "latitude"), crs = 4326)
```

st_as_sf function converts different data types to simple features.
dat is our original data.
coords gives the names of the columns that relate to the spatial coordinates.
...
GCS’s are represented by angular units (i.e. longitude and latitude), usually in decimal degree

An EPSG code is a unique, short-hand code for a specific coordinate reference system (CRS).
...
When using sf we can use all the operations that work on dataframes on sf simple features collections. These include data wrangling operations like inner_join, plotting operations from ggplot2 and model fitting tools too (like glm).

sf also adds geometric operations, like st_join which does joins based on the coordinates. 

# Cartography
sfhas simple plottingfeatures like this:
```{r}
plot(sdat["richness_raw"]) # Plot raw richness data.
```
If we used plot(sdat) it would create a panel for every variable in our dataframe, see below. In sf, we can use square brackets ["richness_raw"] to select a single variable.
...
Like ggplot2 we can change the aesthetics of maps from the different packages available in R.
...
```{r}
# Using tmap
tm_shape(sdat) + # Declare the layer.
  tm_dots(col = "richness_raw") # Plots dots of the coordinates. Select richness as the colour scale.
```

We can use tm_polygons and tm_symbols as an alternative to tm_dots.
...
# Loading shapefiles
We are working with shapefiles in this case study because it is still the most likely format you’ll encounter when someone sends you a spatial dataset.
We can read shapefiles directly into R with the st_read command. 
```{r}
aus <- st_read("../data/data-for-course/spatial-data/Aussie/Aussie.shp") # Read in aus shapefile.

aus # Look at data.

shelf <- st_read("../data/data-for-course/spatial-data/aus_shelf/aus_shelf.shp") # Read in shelf shapefile.
...
```{r}
tm_shape(shelf) + # Declare layer 'shelf'.
  tm_polygons() # Plot polygon of coordinates
```

Now we can build on the map by layering in a similar way as we do in ggplot2.
...
```{r}
tm_shape(shelf, bbox = sdat) + # Use bbox to expand the extent of the map.
  tm_polygons() + 
  tm_shape(aus) + # Add shape.
  tm_polygons() + 
  tm_shape(sdat) + # Add copepod data.
  tm_dots() # Display data points.
```


# Exploring t_map
```{r}
vignette('tmap-getstarted') # This is a built-in workflow in R on using tmap.

# Hello World!
library(tmap)
data("World") # Load data.
tm_shape(World) + # Set shape of data.
  tm_polygons("HPI") # Use HPI to form polygons.
```

# Interactive Maps
```{r}
tmap_mode("view") # Make the map interactive.
...
tm_shape(land) + # Set shape.
  tm_raster("elevation", palette = terrain.colors(10)) + # Set colour.
  tm_shape(World) + # Set shape.
  tm_borders("white", lwd = .5) + # Create border aesthetics.
  tm_text("iso_a3", size = "AREA") + # Add text to map.
  tm_shape(metro) +
  tm_symbols(col = "red", size = "pop2020", scale = .5) + # Set pop2020 for symbols and set aesthetics for symbols.
  tm_legend(show = FALSE) # Don't include legend.
```
...
tm_shape(World) +
  tm_polygons("HPI") +
  tm_layout(bg.color = "skyblue", inner.margins = c(0, .02, .02, .02)) # Set aesthetics.
```
...
```{r}
tmap_options(bg.color = "black", legend.text.color ="white") # Create legned aesthetics.

tm_shape(World) +
  tm_polygons("HPI", legend.title = "Happy Planet Index")# Set aesthetics.
```
